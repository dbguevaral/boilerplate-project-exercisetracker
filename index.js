...
app.post("/api/users/:_id/exercises", (req, res) => {
  ...
  if (typeof description !== "string")
    return res.send("Description must be a string");
  if (description.length > 20)
    return res.send("Description must be at max 20 characters long");
  ...
  if (!date) date = new Date();
    ...
    return res.send("Date must be in YYYY-MM-DD format");
...
app.get("/api/users/:_id/logs", (req, res) => {
  ...
  if (from || to) {
    const fromDate =
      from && moment(from, "YYYY-MM-DD", true).isValid()
    ...
    if ((from && !fromDate) || (to && !toDate)) {
      return res.send(
        'Provided "from" or "to" dates must be in YYYY-MM-DD format'
      );
    }
    log = log.filter((item) => {
  ...
  if (limit) {
    if (!Number.isInteger(Number(limit)))
      return res.send("Limit must be an integer");
